...
export type FileState = {
    ...
    url: string; // used to identify the file
            ...
            <div>
                <div className="flex flex-col gap-2">
                    ...
                    <div className="">
                        ...
                        <p className="text-sm mt-1 text-muted-foreground pl-1">첨부파일은 선택사항입니다</p>
                    ...
                    <div className={`space-y-2 h-auto ${isPostEditPage ? 'max-h-[160px]' : 'max-h-[325px]'} overflow-y-auto mb-[1px]`}>
                        {/* Selected Files */}
                        {value?.map(({file, progress, key, url}) => (
                            <div
                                key={key}
                            ...
                            >
                                    ...
                                    <div className="flex w-12 justify-end text-xs">
                                        {progress === 'PENDING' ? (
                                            ...
                                            <div className="flex flex-1 items-center justify-center gap-x-2">
                                                <CheckCircleIcon
                                                    className="shrink-0 w-6 h-6 text-green-600 dark:text-gray-400"/>
                                                <Separator orientation="vertical" className="h-full"/>
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        const isKeyInKeys = keys?.includes(key); // Check if the key is in the keys array
                                                        if (isKeyInKeys) {
                                                            onDeleteFile?.(url);
                                                            // console.log(url);
                                                            // console.log(urls)
                                ...
                                {typeof progress === 'number' && (
                                    <div className="relative h-0">
                                        <div
                                            className="absolute top-1 h-1 w-full overflow-clip rounded-full bg-gray-200 dark:bg-gray-700">
                                            <div
                                                className="h-full bg-gray-400 transition-all duration-500 ease-in-out dark:bg-white"
                                                style={{
